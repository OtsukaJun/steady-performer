...
function getFirstUsedDate() {
    ...
    if (!firstUsedDate) {
        ...
        const today = new Date();
        firstUsedDate = getTodayDateString(today);
...
function ensureAllDaysRecorded() {
    ...
    const firstDate = new Date(firstUsedDate+'T00:00:00');
    const today = new Date();
...
function startQuiz() {
    ...
    // Build a pool of unique questions (dedupe by `word`), then shuffle for this session
    const seen = new Set();
    const uniqueQuestions = [];
    vocabulary.forEach(q => {
        if (!seen.has(q.word)) {
            seen.add(q.word);
...
function loadQuestion() {
    // If we've run out of questions for this session, end the session and show results
    if (!shuffledQuestions || questionPoolIndex >= shuffledQuestions.length) {
        showResults();
        return;
    }
...
function showResults() {
    ...
    // Update stats with current session values (1回の学習の値)
